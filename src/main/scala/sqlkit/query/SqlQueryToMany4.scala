...
case class SqlQueryToMany4[R, R1, R2, R3, R4](
  query: String,
  rawQuery: String = "",
  ...
  def on(params: SqlParam*): SqlQueryToMany4[R, R1, R2, R3, R4] = {
    this.copy(params = this.params ++ params.toList)
  }

  def list(implicit session: SqlSession) = {
    exec { rs =>
      rs.foldLeft(Map.empty[R, (List[R1], List[R2], List[R3], List[R4])]) { case (all, row) =>
        val o = one(row)
        val (item1, item2, item3, item4) = all.getOrElse(o, (List.empty[R1], List.empty[R2], List.empty[R3], List.empty[R4]))
        all + (
          o -> ((
  ...
  def listSets(implicit session: SqlSession) = {
    list.map { case (k, (toMany1, toMany2, toMany3, toMany4)) =>
      (k, Set.from(toMany1), Set.from(toMany2), Set.from(toMany3), Set.from(toMany4))
    }
  }
}
